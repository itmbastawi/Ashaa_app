...
def addoct():
    ...
    if form.accepted:
        session.flash = 'تم اسم الطبيب بنجاح'
    elif form.errors:
...
def add_expe():
    form=SQLFORM(db.expe).process(next='view_expe/[id]')
    if form.accepted:
...
def view_expe():
    id=request.args(0,cast=int) or redirect(URL('index'))
    rows=db(db.expe.id==id).select()
    return locals()

def edit_expe():
    id=request.args(0,cast=int) or redirect(URL('index'))
    forms=SQLFORM(db.expe,id,deletable = True).process(next='view_expe/[id]')
    if form.accepted:
